...
#   arr = arr [ (lat_start * array_scale) : (lat_end * array_scale),
#               (lon_start * array_scale) : (lon_end * array_scale),
#               redim=FALSE]
  lat1 = (lat_start * array_scale)
  lat2 = (lat_end * array_scale)
  long1 = (lon_start * array_scale)
  long2 = (lon_end * array_scale)
  arr = subset(arr, latitude_e4 >= lat1 & latitude_e4 <= lat2 & longitude_e4 >= long1 & longitude_e4 <= long2)
  # arr = bind(arr, c("lat", "lon"), c("latitude_e4 / 10000.0", "longitude_e4 / 10000.0"))
...
{
  arr = scidb(array_name);
  #arr =  arr[ (lat * array_scale - image_height / 2) : (lat * array_scale + image_height / 2), 
  #            (lon * array_scale - image_width / 2)  : (lon * array_scale + image_width  / 2),
  #            redim=FALSE]
